...
const source = await readFile(resolve(process.cwd(), "contracts/MapPin.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "MapPin.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["MapPin.sol"]?.MapPin;
if (!contract) {
...
console.log(`Deploying MapPin from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_MAP_PIN_CONTRACT_ADDRESS to this contract address in Vercel.");
